...
module Prawn
  class Document
    def styled_text( data )
      pst_element = PrawnStyledText::Element.new
      parts = []
      text_options = {}
      extra_options = { margin_left: 0 }
      oga = Oga.parse_html data
      pst_element.traverse oga.children do |type, text, data|
        context = if type == :text_node
                    pst_element.text_node( self, data )
          elsif type == :opening_tag
            pst_element.opening_tag( self, data )
          else
            pst_element.closing_tag( self, data )
          end
